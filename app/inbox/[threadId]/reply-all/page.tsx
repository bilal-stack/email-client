...
import { dedupeByEmail } from "@/lib/compose/parse-addresses";
import type { CanonicalAddress } from "@/lib/providers/types";
import { notFound } from "next/navigation";

export const dynamic = "force-dynamic";

interface ReplyAllPageProps {
  params: Promise<{ threadId: string }>;
}

export default async function ReplyAllPage({ params }: ReplyAllPageProps) {
  const { threadId } = await params;
  const ctx = await loadParentForCompose(threadId);
  if (!ctx) notFound();

  const ownAddress = ctx.account.emailAddress.toLowerCase();
  const allRecipients: CanonicalAddress[] = [
    ...(ctx.parent.from.email ? [ctx.parent.from] : []),
    ...ctx.parent.to,
  ];
  const filteredTo = dedupeByEmail(allRecipients).filter(
    (a) => a.email.toLowerCase() !== ownAddress,
  );
  const filteredCc = dedupeByEmail(ctx.parent.cc).filter(
    (a) => a.email.toLowerCase() !== ownAddress,
  );
  ...
  const draftRow = await getDraftForUser(ctx.userId, { threadId, mode: "reply-all" });
    ...
    <div className="grid h-full grid-cols-1">
      <section className="flex min-h-screen flex-col">
        <header className="border-b border-zinc-200 bg-white px-4 py-4 sm:px-6">
          <h1 className="truncate text-lg font-semibold text-zinc-900">Reply all</h1>
        ...
        <div className="flex-1">
          <Composer
            ...
            parentMessage={{
              ...
              prefilledTo: filteredTo,
              prefilledCc: filteredCc,
