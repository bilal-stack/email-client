...
import { buildForwardQuote, buildReplyHeaders, prefixForwardSubject } from "@/lib/compose/headers";
...
export default async function ForwardPage({ params }: ForwardPageProps) {
  ...
  const subject = prefixForwardSubject(ctx.parent.subject);
  const forwardQuote = buildForwardQuote({
    from: ctx.parent.from,
    receivedAt: ctx.parent.receivedAt,
    subject: ctx.parent.subject,
    to: ctx.parent.to,
    bodyHtml: ctx.parent.bodyHtml,
    bodyText: ctx.parent.bodyText,
  });

  const draftRow = await getDraftForUser(ctx.userId, { threadId, mode: "forward" });
    ...
    <div className="grid h-full grid-cols-1">
      <section className="flex min-h-screen flex-col">
        <header className="border-b border-zinc-200 bg-white px-4 py-4 sm:px-6">
          <h1 className="truncate text-lg font-semibold text-zinc-900">Forward</h1>
